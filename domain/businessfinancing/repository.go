...
type businessCreditCard struct {
...
func NewRepository(dao *bongo.Collection) Repository {
	return &businessCreditCard{
		dao: dao,
	}
}

//Save save an entity
func (r *businessCreditCard) Save(entity Entity) error {
	return r.dao.Save(&entity)
}

//DeleteMany delete all branches from an institution
func (r *businessCreditCard) DeleteMany(institutionID string) error {
	_, err := r.dao.Delete(bson.M{"institutionid": institutionID})
	return err
}

//FindByInstitution find all businessLoan from an institution
func (r *businessCreditCard) FindByInstitution(institutionID string, page int) ([]Entity, *subentities.Pagination, common.CustomError) {
	...
	businessCreditCards := make([]Entity, info.RecordsOnPage)

	for i := 0; i < info.RecordsOnPage; i++ {
		_ = results.Next(&businessCreditCards[i])
	}

	pagination := subentities.Pagination{Total: info.TotalPages, Current: info.Current}

	return businessCreditCards, &pagination, nil
}
