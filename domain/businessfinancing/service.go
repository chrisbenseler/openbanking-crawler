...
func (s *service) DeleteAllFromInstitution(InstitutionID string) common.CustomError {
	...
	if deleteErr != nil {
		return common.NewInternalServerError("Could not delete businessFinancings from institution", deleteErr)
	}

	return nil
}

//InsertMany insert many businessFinancings in instition
func (s *service) InsertMany(businessFinancings []Entity, institututionID string) common.CustomError {

	for _, businessFinancing := range businessFinancings {
		businessFinancing.InstitutionID = institututionID
		s.repository.Save(businessFinancing)
	}
