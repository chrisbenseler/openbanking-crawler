...
func (s *service) DeleteAllFromInstitution(InstitutionID string) common.CustomError {
	...
	if deleteErr != nil {
		return common.NewInternalServerError("Could not delete personalFinancings from institution", deleteErr)
	}

	return nil
}

//InsertMany insert many personalFinancings in instition
func (s *service) InsertMany(personalFinancings []Entity, institututionID string) common.CustomError {

	for _, personalFinancing := range personalFinancings {
		personalFinancing.InstitutionID = institututionID
		s.repository.Save(personalFinancing)
	}
