...
func (s *service) DeleteAllFromInstitution(InstitutionID string) common.CustomError {
	...
	if deleteErr != nil {
		return common.NewInternalServerError("Could not delete personalCreditCards from institution", deleteErr)
	}

	return nil
}

//InsertMany insert many personalCreditCards in instition
func (s *service) InsertMany(personalCreditCards []Entity, institututionID string) common.CustomError {

	for _, personalCreditCard := range personalCreditCards {
		personalCreditCard.InstitutionID = institututionID
		s.repository.Save(personalCreditCard)
	}
