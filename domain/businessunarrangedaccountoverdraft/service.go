...
func (s *service) DeleteAllFromInstitution(InstitutionID string) common.CustomError {
	...
	if deleteErr != nil {
		return common.NewInternalServerError("Could not delete businessUnarrangedAccountOverdraft from institution", deleteErr)
	}

	return nil
}

//InsertMany insert many businessUnarrangedAccountOverdrafts in instition
func (s *service) InsertMany(businessUnarrangedAccountOverdrafts []Entity, institututionID string) common.CustomError {

	for _, businessUnarrangedAccountOverdraft := range businessUnarrangedAccountOverdrafts {
		businessUnarrangedAccountOverdraft.InstitutionID = institututionID
		s.repository.Save(businessUnarrangedAccountOverdraft)
	}
