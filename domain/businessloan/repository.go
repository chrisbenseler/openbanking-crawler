...
type businessLoanRepository struct {
...
func NewRepository(dao *bongo.Collection) Repository {
	return &businessLoanRepository{
		dao: dao,
	}
}

//Save save an entity
func (r *businessLoanRepository) Save(entity Entity) error {
	return r.dao.Save(&entity)
}

//DeleteMany delete all branches from an institution
func (r *businessLoanRepository) DeleteMany(institutionID string) error {
	_, err := r.dao.Delete(bson.M{"institutionid": institutionID})
	return err
}

//FindByInstitution find all businessLoan from an institution
func (r *businessLoanRepository) FindByInstitution(institutionID string, page int) ([]Entity, *subentities.Pagination, common.CustomError) {
	...
	businessLoans := make([]Entity, info.RecordsOnPage)

	for i := 0; i < info.RecordsOnPage; i++ {
		_ = results.Next(&businessLoans[i])
	}

	pagination := subentities.Pagination{Total: info.TotalPages, Current: info.Current}

	return businessLoans, &pagination, nil
}
